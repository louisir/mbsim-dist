...
MBSim 자체 애플리케이션 코드와 문서는 무료로 사용할 수 있는 독점 소프트웨어로 제공됩니다. 공식 MBSim 릴리스 패키지는 무료로 설치하고 사용할 수 있습니다. MBSim은 오픈소스 소프트웨어가 아니며 소스 코드는 공개되지 않습니다. [LICENSE](LICENSE.ko.md)를 참조하십시오.

후원이 제공되는 경우 이는 향후 개발을 위한 자발적 지원입니다. 후원은 라이선스 조건을 변경하지 않으며 지원, 보증, 서비스 또는 유지보수 의무를 만들지 않습니다.

MBSim freeware 라이선스는 MBSim 자체 코드와 문서에만 적용됩니다. 포터블 패키지에 포함된 타사 런타임 라이브러리를 재라이선스하지 않습니다.
...
- MBSim freeware 라이선스는 Qt 라이선스 요구 사항을 제거하거나 약화하지 않습니다.
...
1. MBSim 라이선스를 패키지에 포함하십시오.
2. 타사 고지와 라이선스 텍스트를 패키지에 포함하십시오.
3. MBSim 자체를 재배포하거나 제품에 포함하는 경우, 강행 법규가 달리 정하지 않는 한 권리자의 서면 허가를 받으십시오.
4. 빌드에 포함된 정확한 Qt 모듈에 대해 현재 Qt 라이선스 페이지를 확인하십시오.
5. 오픈소스 Qt와 GPL-only Qt 모듈로 빌드된 패키지를 배포하는 경우, 배포가 GPLv3를 준수하는지 확인하거나 Qt 상용 라이선스를 사용하십시오.
