...
原始碼在獨立倉庫中維護。一般使用者應從 GitHub Releases 下載發布包，而不是 clone
本倉庫。
...
MBSim 自身的應用程式碼和文件使用 MIT License 發布，見 [LICENSE](LICENSE)。

免安裝包中還包含第三方執行時元件。這些第三方元件的授權獨立於 MBSim 的 MIT
License，使用或再次分發發布包時需要同時遵守對應授權。尤其需要注意：
...
詳細說明和官方授權連結見
[THIRD-PARTY-NOTICES.zh_TW.md](THIRD-PARTY-NOTICES.zh_TW.md)。

本倉庫不提供法律意見。如果要在商業產品或受控環境中分發 MBSim，請結合實際發布
方式諮詢專業法律意見。
